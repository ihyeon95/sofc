...
<script type="text/javascript">
	...
	function fnClickSite(iSiteNum, sSiteName){
		
		$("#iSiteNum").val(iSiteNum);
		$("#sSiteName").val(sSiteName);
		
		$("#siteForm").attr({action:'<c:url value="/dashboard/system/mng"/>', method:'post'}).submit();
	}
</script>

<div class="pull-left">
	<button type="button" class="btn btn-info btn-xs mb-3" data-toggle="modal" data-target="#siteInfoModal">추가</button>
</div>
        	...
        	<c:choose>
				...
				<c:otherwise>
					<c:forEach var="siteList" items="${siteList}" varStatus="status">
						<tr>
							<th scope="row" style="word-break: keep-all;"><a href="javascript:fnClickSite(${siteList.iSiteNum}, '${siteList.sSiteName}');"><c:out value="${siteList.sSiteName}"/></a></th>
							<td><c:out value="${siteList.sSysCnt}"/></td>
							<td><c:out value="${siteList.fCapa}"/></td>
							<td><fmt:formatNumber value="${siteList.sUsePer}" pattern="###"/></td>
							<td><i class="ti-trash" onclick="javascript:fnDeleteSiteInfoCheck(${siteList.iSiteNum}, '${siteList.sSiteName}');"></i></td>

...
<body>
	...
	<form id="areaForm">
		...
		<input type="hidden" id="iAreaNum" name="iAreaNum">
...
<%--					</ul>--%>
				...
				<div id="content">
					<p class="breadcrumb_navi">
						...
						<span class="current">${sCityName}</span>
					</p>
					...
					<div class="sales-report-area mt-5 mb-5">
						<div class="row">
							...
							<div class="col-md-2">
								<div class="card">
									<div class="card-body">
										<h4 class="header-title">도시 운영현황</h4>
										<div class="trad-history">
										    <div class="tab-content" id="myTabContent">
										        <div class="tab-pane fade show active" id="buy_order" role="tabpanel">
										            <div class="table-responsive">
							...
							<div class="col-md-3">
							    <div class="card">
							        <div class="card-body" style="overflow:auto;">						            
							            <h4 class="header-title mb-0">도시 발전량 그래프</h4>
			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-row">
							<div class="col-md-12 mb-3">
							    <label for="iRtuNum" class="col-form-label">지역명</label>
							    <input class="form-control" type="text" value="" id="sAreaNameText">
							</div>
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary" onclick="javascript:fnAreaInfoInsert();">저장</button>
					</div>
				</div>
			</div>
		</div>
		<div class="modal fade bd-example-modal-sm" id="areaDeleteCheckModal">
			<div class="modal-dialog modal-sm">
				<div class="modal-content">
					...
					<div class="modal-body">
						<div id="sAreaNameDel"></div> 지역을 삭제하시겠습니까?
						<input type="hidden" id="iSysNum">
					</div>
					<div class="modal-footer">
					    <button type="button" class="btn btn-secondary" data-dismiss="modal">취소</button>
					    <button type="button" class="btn btn-primary" onclick="javascript:fnDeleteAreaInfo();">삭제</button>
					</div>
				</div>
			</div>
		</div>
				...
				$.ajax({
				    ...
				    ,error : function(xhr,status,error) {
				        ...
				        if(xhr.status == 401) {
				            ...
				            location.href = "/dashboard/area/mng";
		...
		function fnMeasureSearch() {
			...
			$.ajax({
				type : 'post',
				url : '/dashboard/area/measureList',
				data : params,
				global : false,
				dataType : 'JSON',
				beforeSend : function(xhr) {
		...
		function fnAreaInfoInsert(){
			
			if($('#sAreaNameText').val() == "") {
				alert("값을 모두 입력해주세요.");
			}else{
				var params = {
						"iCityNum" : $('#iCityNum').val(),
						"sAreaName" : $('#sAreaNameText').val()
				};
				
				$.ajax({
					type : 'post',
					url : '/dashboard/area/insertAreaInfo',
					data : params,
					global : false,
					dataType : 'JSON',
					beforeSend : function(xhr) {
		...
		function fnDeleteAreaInfoCheck(iAreaNum, sAreaName){
			
			$('#sAreaNameDel').html(sAreaName);
			$('#iAreaNum').val(iAreaNum);
			
			$('#areaDeleteCheckModal').modal();
			
		}
		
		function fnDeleteAreaInfo(){
			...
			$.ajax({
				type : 'post',
				url : '/dashboard/area/deleteAreaInfo',
				data : params,
				global : false,
				dataType : 'JSON',
				beforeSend : function(xhr) {
				...
				success : function(data) {
					if(data.result == "success") {
						...
						$('#areaDeleteCheckModal').modal('hide');

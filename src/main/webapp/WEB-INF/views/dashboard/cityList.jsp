...
<script type="text/javascript">
	...
	function fnClickCity(iCityNum, sCityName){
		
		$("#iCityNum").val(iCityNum);
		$("#sCityName").val(sCityName);
		
		$("#cityForm").attr({action:'<c:url value="/dashboard/area/mng"/>', method:'post'}).submit();
	}
</script>
        	...
        	<c:choose>
				...
				<c:otherwise>
					<c:forEach var="cityList" items="${cityList}" varStatus="status">
						<tr>
							<th scope="row" style="word-break: keep-all;"><a href="javascript:fnClickCity(${cityList.iCityNum}, '${cityList.sCityName}');"><c:out value="${cityList.sCityName}"/></a></th>
							<td><c:out value="${cityList.sSysCnt}"/></td>
							<td><c:out value="${cityList.fCapa}"/></td>
							<td><fmt:formatNumber value="${cityList.sUsePer}" pattern="###"/></td>
							<c:if test="${sAuth eq 'ROLE_SU'}">
								<td><i class="ti-trash" onclick="javascript:fnDeleteCityInfoCheck(${cityList.iCityNum}, '${cityList.sCityName}');"></i></td>		
							</c:if>
						</tr>
					</c:forEach>
				</c:otherwise>
